...
try {
    ...
    try {
        ...
        if ($user['role'] === 'designer') {
            ...
            $stmt = $pdo->prepare('SELECT image_path FROM portfolio_images WHERE designer_id = ?');
            $stmt->execute([$userId]);
            ...
            $stmt = $pdo->prepare('DELETE FROM portfolio_images WHERE designer_id = ?');
            $stmt->execute([$userId]);
    ...
    echo json_encode([
        'success' => false,
        'error' => 'Erro inesperado ao excluir conta',
