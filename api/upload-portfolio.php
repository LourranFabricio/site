...
$uploadDir = __DIR__ . '/../assets/img/portfolio/';                    // Diretório de destino
...
try {
    ...
    $stmt = $pdo->prepare('INSERT INTO PortfolioImages (designer_id, image_path, ordem) VALUES (?, ?, ?)');
    
    // Insere cada arquivo no banco com ordem sequencial
    foreach ($uploadedFiles as $index => $filepath) {
        $stmt->execute([$_SESSION['user']['id'], $filepath, $index + 1]);
    }
